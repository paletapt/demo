node {
	...
	stage('Build + SonarQube analysis') {
     ...
     withSonarQubeEnv('My SonarQube Server') { 
      ...
      bat "\"${tool 'MSBuild'}\\MSBuild.exe\" ${WORKSPACE}\\testePipeline\\ContactManager.sln /p:Configuration=Release /p:Platform=\"Any CPU\" /p:ProductVersion=1.0.0.${env.BUILD_NUMBER}"
